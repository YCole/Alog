...
import com.mediatek.mtklogger.taglog.LogInformation;
import com.mediatek.mtklogger.taglog.TagLogData;
import com.mediatek.mtklogger.taglog.TagLogInformation;
...
public class DBManager {
    private static final String TAG = Utils.TAG + "/DBManager";
     ...
     * @return long
     */
    public long insertTaglogToDb(TagLogInformation taglogInfo) {
        return sDataBaseHelper.insertTaglogInfo(
                translateToTaglogTable(taglogInfo));
    }
    /**
     * @param taglogInfo TaglogTable
     * @return long
     */
    public long insertTaglogToDb(TaglogTable taglogInfo) {
        return sDataBaseHelper.insertTaglogInfo(taglogInfo);
    }
    /**
     * @param taglogId long
    ...
    public List<TagLogData> getResumeTaglog() {
        String selectStr = MySQLiteHelper.COLUMN_STATE + " != " + " '"
                           + MySQLiteHelper.TAGLOG_STATE_DONE + "'" ;
     ...
     */
    public void typelogDone(LogInformation logInfo) {
        FileInfoTable fileInfo = logInfo.getFileInfo();
        if (fileInfo == null) {
            Utils.logw(TAG, "typelogDone, just return! for fileInfo = null");
            return;
        }
        DBManager.getInstance().updateFileInfo(fileInfo.getFileId(),
                MySQLiteHelper.FILEINFO_STATE_DONE, fileInfo.getFileCount());
    }
