...
public class MySQLiteHelper {
    private static final String TAG = Utils.TAG + "/DatabaseManagerHelper";
...
//    public static final String COLUMN_STATE = "state";
    ...
    /**
     * Inser taglogtable into db.
     * @param taglogtable TaglogTable.
     * @return result
     */
    public synchronized long insertTaglogInfo(TaglogTable taglogtable) {
        if (null == taglogtable) {
        ...
        contentValues.put(COLUMN_DB_PATH, taglogtable.getDBPath());
        ...
        try {
            ...
            Utils.logw(TAG, "insert fail, for database or disk is full");
        } catch (SQLiteException exp) {
            Utils.logw(TAG, "insert fail, for SQLiteException happened!");
        }
        if (result <= 0) {
            Utils.loge(TAG, "Fail to insertTaglogInfo record: " + taglogtable.toString());
        }
    ...
    public synchronized String insertFileInfo(List<FileInfoTable> fileinfoList) {
        ...
        for (FileInfoTable fileInfo : fileinfoList) {
            ...
            contentValues.put(COLUMN_ORIGINAL_PATH, fileInfo.getOriginalPath());
            contentValues.put(COLUMN_SOURCE_PATH, fileInfo.getSourcePath());
            ...
            if (result <= 0) {
                Utils.loge(
                        TAG, "Fail to instert fileinfo record: " + fileInfo.toString());
            } else {
    ...
    public List<TaglogTable> queryTaglogTable(String selectionStr) {
        ...
        Cursor cursor = db.query(TAGLOG_TABLE, null, selectionStr, null,
                null, null, null);
        if (cursor == null) {
            Utils.logw(TAG,
    ...
    public List<FileInfoTable> queryFileInfoTable(String selectionStr) {
        ...
        SQLiteDatabase db = sSqliteHelper.getReadableDatabase();
        Cursor cursor = db.query(FILEINFO_TABLE, null, selectionStr, null,
                null, null, null);
        if (cursor == null) {
            Utils.logw(TAG,
...
//    /**
//     * Drop table when re-sync info.
//     * @param table with to delete.
//     */
//    private void dropTable(String table) {
//        Utils.logi(TAG, "begin to dropTable table: " + table);
//        String dropTableSqlStr = "drop table if exists " + table;
//        sSqliteHelper.getWritableDatabase().execSQL(dropTableSqlStr);
//    }
    /**
