...
public class LogConfig {
    ...
    private static final String CONFIG_FILE_SUFFIX =
            "shared_prefs/com.mediatek.mtklogger_preferences.xml";
    ...
    private static String sConfigFilePathRoot = File.separator
            + "data/data/com.mediatek.mtklogger/";

    private LogConfig() {
        String path = mContext.getFilesDir().getAbsolutePath();
        Utils.logi(TAG, "APK private file root path = " + path);
        if (!TextUtils.isEmpty(path) && path.contains("files")) {
            sConfigFilePathRoot = path.substring(0, path.indexOf("files"));
        }
        Utils.logi(TAG, "APK private file root sConfigFilePathRoot = " + sConfigFilePathRoot);
    }
        ...
        if (new File(configFile).exists()) {
            ...
            try {
                ...
                for (int logType : Utils.LOG_TYPE_SET) {
                    ...
                    boolean readUIEnabled = true;
                    String readUIEnabledValue = customizeProp
                            .getProperty(Utils.KEY_CONFIG_LOG_UI_ENABLED_MAP.get(logType));
                    ...
                    if (readUIEnabledValue != null) {
                        readUIEnabled = Boolean.parseBoolean(readUIEnabledValue);
                    }
                    if (readLogSize != null) {
                        try {
                ...
                if (alwaysTagModemLogEnableValue != null) {
                    editor.putBoolean(SettingsActivity.KEY_ALWAYS_TAG_MODEM_LOG_ENABLE,
                            Boolean.parseBoolean(alwaysTagModemLogEnableValue)).apply();
                }
                ...
                String notificationEnableValue = customizeProp
                        .getProperty(Utils.KEY_CONFIG_NOTIFICATION_ENABLED);
                Utils.logd(TAG, "Init log config, notificationEnableValue ? "
                        + notificationEnableValue);
